...
def fastrcnn_loss(class_logits,
    ...
    item_loss_weight = class_weight_map[labels]
    classification_loss = F.cross_entropy(class_logits,
                                          labels,
                                          reduction="none")
    classification_loss = torch.dot(classification_loss,
                                    item_loss_weight) / labels.numel()
    ...
    item_loss_weight_pos = class_weight_map[labels_pos]
    box_loss = torch.dot(box_loss.sum(dim=1),
                         item_loss_weight_pos) / labels.numel()
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels,
                  mask_matched_idxs, class_weight_map):
    ...
    mask_loss = F.binary_cross_entropy_with_logits(
        mask_logits[torch.arange(labels.shape[0], device=labels.device),
                    labels],
        mask_targets,
        reduction="none")
    #import pdb; pdb.set_trace()
    item_loss_weight_pos = class_weight_map[labels]
    #item_loss_weight_pos = torch.tensor([
    #    class_weight_map[i.item()] for i in labels
    #]).to(labels.device).to(torch.float)
    return torch.dot(mask_loss.mean(dim=[1, 2]),
                     item_loss_weight_pos) / item_loss_weight_pos.numel()
...
def _onnx_heatmaps_to_keypoints_loop(maps, rois, widths_ceil, heights_ceil,
                                     widths, heights, offset_x, offset_y,
                                     num_keypoints):
    xy_preds = torch.zeros((0, 3, int(num_keypoints)),
                           dtype=torch.float32,
                           device=maps.device)
    end_scores = torch.zeros((0, int(num_keypoints)),
                             dtype=torch.float32,
                             device=maps.device)

    for i in range(int(rois.size(0))):
        xy_preds_i, end_scores_i = _onnx_heatmaps_to_keypoints(
            maps, maps[i], widths_ceil[i], heights_ceil[i], widths[i],
            heights[i], offset_x[i], offset_y[i])
        xy_preds = torch.cat((xy_preds.to(dtype=torch.float32),
                              xy_preds_i.unsqueeze(0).to(dtype=torch.float32)),
                             0)
        end_scores = torch.cat(
            (end_scores.to(dtype=torch.float32),
...
def heatmaps_to_keypoints(maps, rois):
    ...
    for i in range(len(rois)):
        ...
        roi_map = F.interpolate(maps[i][:, None],
                                size=(roi_map_height, roi_map_width),
                                mode="bicubic",
                                align_corners=False)[:, 0]
        ...
        xy_preds[i, 2, :] = 1
        end_scores[i, :] = roi_map[
            torch.arange(num_keypoints, device=roi_map.device), y_int, x_int]

    return xy_preds.permute(0, 2, 1), end_scores


def keypointrcnn_loss(keypoint_logits, proposals, gt_keypoints,
...
def _onnx_expand_boxes(boxes, scale):
    ...
    boxes_exp = torch.stack((boxes_exp0, boxes_exp1, boxes_exp2, boxes_exp3),
                            1)
    return boxes_exp
...
def expand_masks_tracing_scale(M, padding):
    # type: (int, int) -> float
    return torch.tensor(M + 2 * padding).to(
        torch.float32) / torch.tensor(M).to(torch.float32)
...
def expand_masks(mask, padding):
    ...
    ):  # could not import is_tracing(), not sure why
...
def _onnx_paste_mask_in_image(mask, box, im_h, im_w):
    ...
    concat_0 = torch.cat(
        (zeros_y0, unpaded_im_mask.to(dtype=torch.float32), zeros_y1),
        0)[0:im_h, :]
    # pad x
...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                match_quality_matrix = box_ops.box_iou(gt_boxes_in_image,
                                                       proposals_in_image)
    ...
    def check_targets(self, targets):
        ...
        if not all(["boxes" in t for t in targets]):
            raise ValueError(
                "Every element of targets should have a boxes key")
        if not all(["labels" in t for t in targets]):
            raise ValueError(
                "Every element of targets should have a labels key")
        if self.has_mask():
            if not all(["masks" in t for t in targets]):
                raise ValueError(
                    "Every element of targets should have a masks key")
    ...
    def forward(
        ...
        if (self.keypoint_roi_pool is not None
            ...
            if self.training:
                ...
                if matched_idxs is None:
                    raise ValueError(
                        "if in trainning, matched_idxs should not be None")
